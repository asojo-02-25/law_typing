...
// curretnQuestion.kanaであるkanaSourceの形式を文字列を含む配列に統一し、かつ空白を除くように整える処理
...
// 何も入力していない段階のローマ字候補をセット→この先入力に応じて絞っていく
...
const updateChunkIndexFromState = (force = false) => {
    // buildUnitsFromChunks~primeTypingStateFromKanaで作成されたunitChunkMapを利用してユニットの通しのインデックスと文節のインデックスを対応させている
    const mappedIndex = unitChunkMap[typingState.currentUnitIdx]; 
    console.log('[ChunkIndexUpdate] mappedIndex:', mappedIndex, 'currentUnitIdx:', typingState.currentUnitIdx);
    // mappedIndexが数値であればnormalizedIndexに代入、そうでなければchunkedText.length(配列の要素数)を代入
...
// unitとunitが属するchunkのインデックスの対応配列を作成する準備
// fallback : 代わりに頼るもの chunksが正しく生成されたらchunksを採用、もし空ならfallbackTextをchunksとする
const buildUnitsFromChunks = (chunks, fallbackText = '') => {
    ...
    if (Array.isArray(chunks) && chunks.length > 0) {
        ...
        // chunksが空のとき、fallbackTextをユニットに分割してunitsのから配列にpushする
        const parsed = parseKanaUnits(fallbackText);
        parsed.forEach((unit) => {
            units.push(unit);
            // chunksは必ず空なのでchunkMapのインデックスは必ず0になる
            chunkMap.push(0);
        });
    }
    console.log('[buildUnitsFromChunks] units:', units, 'chunkMap:', chunkMap);
...
const primeTypingStateFromKana = (kanaSource, kanaChunks) => {
    ...
    // 動作上の無駄はあるがバグにはならない。関数の独立性、安全性を重視するなら残す、呼び出し元をsetupQuestionDataに限定するなら冗長さを防ぐために削除するのも一案。
    const kanaList = normalizeKanaSource(kanaSource);
    // normalizeKanaSourceはチャンクを作成するものではないため、kanaList[0]はkanaSourceそのものを持つことになる。従ってfallbackとして最適である。
    const primaryKana = kanaList[0] || '';
    // 分割代入　buildUnitsFromChunksから返されるオブジェクトのunitsプロパティとchunkMapプロパティをそれぞれunitsとchunkMapという定数に代入
    const { units, chunkMap } = buildUnitsFromChunks(
        //　chunkedKanaの句読点はkanaプロパティから、chunkedTextはtextプロパティから生成しているため両者の長さが異なる可能性があるためここで比較を行う必要がある。
        Array.isArray(kanaChunks) && kanaChunks.length === chunkedText.length ? kanaChunks : [],
        // buildUnitsFromChunksのfallbackText引数に相当。
        primaryKana
    );
    // すでに定義済みのグローバル変数にchunkMap(unitが属するchunkのインデックスを示す配列)をセット
...
const resetGame = () => {
    ...
    chunkedText = [];           // 日本語(漢字込み)を読点で区切ったリスト
